...
export const GooDefense: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Base spawns at 1.5s. Every 10 points it gets 5% faster, capping locally at 400ms.
  ...
  const fireBlastVisually = (clientX: number, clientY: number) => {
    ...
    setBlasts(prev => [...prev, { id: blastId, x, y }]);

    setTimeout(() => {
      setBlasts(prev => prev.filter(l => l.id !== blastId));
    }, 150);
  };

  const handleShootTarget = (targetId: string, clientX: number, clientY: number) => {
    fireBlastVisually(clientX, clientY);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 mb-12">
      <div className="w-full bg-[#0b0b1e]/60 backdrop-blur-md border border-[#6C7AE0]/30 rounded-xl shadow-[0_0_30px_rgba(108,122,224,0.15)] overflow-hidden">

        {/* Game Header */}
        <div className="bg-black/40 border-b border-[#6C7AE0]/20 p-4 flex justify-between items-center relative z-20">
          <div>
            <h3 className="text-xl font-bold text-[#9AA9FF] display-font uppercase tracking-wider flex items-center gap-3">
              Carrot Defense
              {isPlaying && !gameOver && (
                <button
                  onClick={togglePause}
          ...
          <div className="flex gap-6 items-center">
            <div className="text-center">
              <p className="text-xs text-[#9AA9FF] uppercase tracking-widest">Score</p>
            ...
            {mode === 'survival' && (
              <div className="text-center">
                ...
                <div className="flex gap-1 justify-center mt-1">
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className={`w-3 h-3 rounded-full transition-colors ${i < lives ? 'bg-[#6C7AE0] shadow-[0_0_8px_#6C7AE0]' : 'bg-white/10'}`} />
                  ))}
                </div>
              </div>
            )}
        ...
        <div
          ref={containerRef}
          className={`relative w-full h-[60vh] max-h-[550px] min-h-[350px] overflow-hidden bg-gradient-to-b from-black to-[#0a0a20] transition-all duration-300 touch-none ${(!isFrozen && !gameOver && isPlaying) ? 'cursor-crosshair' : ''}`}
        ...
        >
          ...
          {(!isPlaying || gameOver) && !showLeaderboard && (
            <div className="absolute inset-0 z-30 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center animate-fade-in overflow-y-auto">
              {gameOver ? (
                <>
                  <h2 className="text-5xl font-bold text-[#E74C3C] display-font mb-2 drop-shadow-[0_0_15px_rgba(231,76,60,0.8)] mt-8">VEGGIE OVERLOAD</h2>
                  <p className="text-lg text-white mb-6">Final Score: <span className="text-[#9AA9FF] font-bold text-2xl">{score}</span></p>

                  {mode === 'survival' && score > 0 && !scoreSubmitted && (
                    <form onSubmit={submitScore} className="mb-8 w-full max-w-sm">
                      <p className="text-sm text-[#C7D0FF] mb-3 uppercase tracking-wider">Submit to Global Leaderboard</p>
                      <div className="flex">
                        <input
                          type="text"
                          maxLength={15}
                          ...
                          placeholder="Your Name (e.g. Goober)"
                          className="bg-white/5 border border-white/20 text-white px-4 py-2 rounded-l focus:outline-none focus:border-[#9AA9FF] w-full"
                        />
                        <button
                          type="submit"
                          disabled={isSubmittingScore}
                          className="bg-[#6C7AE0] text-white font-bold px-4 rounded-r uppercase tracking-wider hover:bg-[#9AA9FF] transition-colors"
                        >
                          Submit
                        </button>
                      </div>
                  ...
                  {scoreSubmitted && (
                    <p className="text-[#2ECC71] font-bold mb-8 animate-pulse tracking-widest uppercase">Score Stored in the Goo</p>
                  )}
                </>
              ) : (
                <>
                  <h2 className="text-4xl font-bold text-white display-font mb-4 mt-8">The Carrots <br/><span className="text-[#E74C3C]">Are Coming</span></h2>
                  <p className="text-white/70 mb-8 max-w-sm">
                    B.O.B. hates being forced to eat carrots. They are falling from the sky. Zap them with goo before they reach him. Difficulty scales aggressively.
                  </p>
                </>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                ...
                <button
                  ...
                  className="px-6 py-3 bg-[#6C7AE0]/20 hover:bg-[#6C7AE0]/40 border border-[#6C7AE0] text-[#9AA9FF] font-bold rounded uppercase tracking-wider transition-all"
                >
                  Endless Zen
                </button>
              </div>
              ...
              <button
                ...
                className="text-[#9AA9FF] hover:text-white underline tracking-widest text-sm uppercase transition-colors"
              >
          ...
          {showLeaderboard && (
            <div className="absolute inset-0 z-30 bg-black/95 backdrop-blur-md flex flex-col p-6 animate-fade-in overflow-y-auto">
              <div className="flex justify-between items-center mb-6 border-b border-[#6C7AE0]/20 pb-4 mt-4">
                <div className="flex items-center gap-3">
                  <svg className="w-6 h-6 text-[#9AA9FF]" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                  <h3 className="text-2xl font-bold text-[#9AA9FF] display-font uppercase tracking-widest">The Goo Records</h3>
                </div>
              ...
              <div className="flex-grow overflow-y-auto w-full max-w-md mx-auto pr-2 custom-scrollbar">
                {leaderboard.length === 0 ? (
                  <p className="text-white/40 text-center italic py-10">No records found. B.O.B. probably ate them.</p>
                ) : (
                    ...
                    {leaderboard.map((entry, i) => (
                      <div key={i} className="flex justify-between items-center p-3 rounded bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                        <span className="font-bold text-white/90 flex items-center gap-3">
                          <span className={`text-base ${i === 0 ? 'text-[#9AA9FF]' : (i === 1 ? 'text-gray-300' : (i === 2 ? 'text-orange-400' : 'text-white/30'))}`}>#{i + 1}</span>
                          {entry.name}
                        </span>
                        <span className="text-[#9AA9FF] font-mono tracking-wider">{entry.score}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          {targets.map(target => (
            <div
              ...
              className="absolute top-[-80px] w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center z-10 transition-transform"
              style={{
                left: `${target.left}%`,
            ...
            >
              <span className="text-5xl sm:text-6xl drop-shadow-[0_0_15px_rgba(231,76,60,0.6)] pointer-events-none select-none">🥕</span>
            </div>
          ))}
          ...
          {blasts.map(blast => {
            ...
            const length = Math.sqrt(Math.pow(blast.x - startX, 2) + Math.pow(blast.y - startY, 2));
            const angle = Math.atan2(blast.y - startY, blast.x - startX) * (180 / Math.PI);

            return (
              <div
                key={blast.id}
                className="absolute z-15 bg-gradient-to-r from-white via-[#9AA9FF] to-[#6C7AE0] rounded-full pointer-events-none"
                style={{
                  left: `${startX}px`,
                  top: `${startY}px`,
                  width: `${length}px`,
                  ...
                  boxShadow: '0 0 10px #6C7AE0, 0 0 20px #9AA9FF',
                  opacity: 0.9,
                }}
              />
            );
          ...
          <div className="absolute bottom-[-10px] left-1/2 -translate-x-1/2 z-20 pointer-events-none">
            {isPlaying && !gameOver && !isFrozen && (
              <div className="absolute inset-0 bg-[#6C7AE0] blur-3xl rounded-full opacity-20 animate-pulse" />
            )}
            <img
              src="/bob/bob_main.webp"
              alt="B.O.B. Defender"
              className={`w-40 h-auto drop-shadow-[0_0_15px_rgba(108,122,224,0.5)] transition-transform duration-100 ${blasts.length > 0 ? 'scale-110 brightness-150' : ''}`}
            />
          </div>
