...
export async function* askBobStream(question: string, history: ChatMessage[] = [], signal?: AbortSignal): AsyncGenerator<StreamUpdate, void, unknown> {
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        yield { type: 'error', error: "b.o.b went to take a nap. come back later." };
        return;
      }
    ...
    if (error.name === 'AbortError') {
      console.log('B.O.B. stream aborted gracefully.');
      return;
    }
    console.error("Error asking B.O.B.:", error);
    yield { type: 'error', error: error?.message || "b.o.b wandered off to find a snack. he cannot be reached at this time." };
  } finally {
    if (reader) {
      try {
        await reader.cancel();
